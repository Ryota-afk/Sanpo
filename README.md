...
- **候補ルートを選んだ瞬間に「進行中」として即記録**(status: in_progress)。
  歩行中にブラウザを閉じても消えない。「このルートを歩いた」で完了(completed)にする
- 履歴タブから進行中ルートを**「続ける」で再開**可能(現在地・道順・横断歩道の画面に戻れる)
- 履歴一覧(直近10件・FIFO) / 設定(歩行ペース・被り率デフォルト)画面
